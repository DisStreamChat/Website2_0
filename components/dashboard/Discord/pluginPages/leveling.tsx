...
const marks = [...Array(7)].map((item, index) => ({
    value: index / 2,
    label: `x${index / 2}`,
}));
...
const defaultSettings = (): settings => ({
    ...
    general: {
        ...
        channel: null,
        announcement: false,
        embedMessageData: defaultEmbedGenerator("Congrats {player}, you leveled up to level {level}!"),
    },
    scaling: {
        general: 1,
    },
...
const Leveling = () => {
    ...
    const [state, dispatch] = useReducer<
        ...
        settings
    >(settingsReducer, defaultSettings(), defaultSettings);

    const [defaultValues, setDefaultValues] = useState<settings>(() =>
        defaultSettings()
    );
    const { allChannels, roles, emotes, isPremium, setPremiumModalOpen } =
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                let levelingDocs = levelingCollection.docs.reduce(
                    (acc, cur) => {
                        acc[cur.id] = cur.data();
                        return acc;
                    },
                ...
                const levelSettings = [
                    ...
                    if (key === "general") {
                        ...
                        value.announcement =
                            value.announcement ?? legacyAnnouncment;
                    }
    ...
    const announcementChannel = allChannels.find(
        (channel) => channel.id === state.general.channel
    );

    const noXpRoles = state.bannedItems.roles.map((roleId) =>
        ...
        <div>
            <PluginSubHeader>
                <span>
                    ...
                    <h4>
                        Whenever a user gains a level, DisStreamBot can send a
            ...
            >
                ...
                <div style={{ zIndex: 100 }}>
                    ...
                    <div>
                        ...
                        <Switch
                            color="primary"
                            checked={!!state.general.embedMessage}
                            onChange={(e) => {
                                if (!isPremium) {
                                ...
                                dispatch({
                                    type: actions.UPDATE,
                                    key: "general.embedMessage",
                                    value: e.target.checked,
                                });
                            }}
                        />
                    </div>
                    {!state.general.embedMessage ? (
                        <>
                            <EmoteParent>
                                <EmotePickerOpener
                                    onClick={() => {
                                ...
                                <EmotePicker
                                    ...
                                    onEmoteSelect={(emote) => {
                                        const position =
                                            textAreaRef.current.getCaretPosition();
                        ...
                        <EmbedEditor
                            value={state.general.embedMessageData}
                            onChange={(value) => {
                                dispatch({
                                    type: actions.UPDATE,
                                    key: "general.embedMessageData",
            ...
            <PluginSubHeader>
                <span>
                    ...
                    <h4>
                        Change the rate at which all members of the server gain
                        XP.
                    </h4>
                </span>
            </PluginSubHeader>
            ...
            <PluginSubHeader>
                <span>
                    <H2>No-XP Roles</H2>
                    <h4>
                        You can set roles here that will stop users from gaining
                        XP.
                    </h4>
                </span>
            </PluginSubHeader>
            ...
            <PluginSubHeader>
                <span>
                    ...
                    <h4>
                        You can also prevent your members from gaining XP if
                        they send messages in certain text channels.
                    </h4>
                </span>
            </PluginSubHeader>
            <PluginSection>
