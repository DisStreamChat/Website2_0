    ...
    return (
        <Modal open={props?.open} onClose={props.onClose}>
            <CommandModalBody>
                <CommandHeader>
                    <H2>
                        {defaultValue ? "Edit" : "Create"} Reaction Role Message
                    </H2>
                ...
                <CreateCommandArea className="small">
                    ...
                    {formErrors.channel === "Required" && (
                        <Error>Please Select a channel</Error>
                    )}
                    <hr></hr>
                    ...
                    <SectionSubtitle>
                        A catch all reaction will run when any reaction is made,
                        while a regular reaction will only run on when its emoji
                        is reacted with
                    </SectionSubtitle>
                    <ButtonContainer>
                        <div style={{ position: "relative" }}>
                            <BlueButton
                                onClick={() =>
                                    setCreatingReaction({ type: "normal" })
                                }
                            >
                                Add a Reaction
                            </BlueButton>
                                ...
                                >
                                    ...
                                    <div style={{ minWidth: "50%" }}>
                                        ...
                                        {formErrors[emote] === "Required" && (
                                            <Error>
                                                Please add atleast one role
                                            </Error>
                                        )}
                                        {roleErrors[emote] && (
                                            <Warning>
                                                Whoops, it looks like I can't
                                                give one of the roles listed
                                                here. Please fix that by putting
                                                my role above all the roles
                                                listed here, or remove any roles
                                                above mine from the list.
                                            </Warning>
                                        )}
                                    </div>
                                    <div style={{ minWidth: "50%" }}>
...
const RoleManagement = () => {
    ...
    const changed = !isEqual(localSettings, {
        reactions,
        commands,
        join,
        descriptions,
    ...
    const roleOptions = notEveryone.filter(
        (role) =>
            descriptions?.roles?.[TransformObjectToSelectValue(role)] ===
            undefined
    );
        ...
        <>
            ...
            >
                ...
                <CommandsHeader>
                    <span>
                        ...
                        <h4>
                            allow users to give/remove roles from themselves by
            ...
            >
                ...
                <CommandsHeader>
                    <span>
                        <H2>Role Command</H2>
                        <h4>
                            allow users to give/remove roles from themselves
                            with a command
                        </h4>
                    </span>
                    <span>
                ...
                <span>
                    <H2>
                        Commands -{" "}
                        {Object.entries(commands.commands || {})?.length || 0}
                    </H2>
                    <ul>
                        {Object.entries(commands.commands || {}).map(
                            ([key, val]) => (
                                <ListItem
                                    delete={() => {
                                        docRef.update({
            ...
            >
                <Select
                    ...
                    options={notManaged
                        ?.filter(
                            (role) =>
                                !join.roles?.find(({ id }) => role.id == id)
                        )
                        ?.map((role) => ({
                            value: TransformObjectToSelectValue(role),
                            label: (
                                <RoleOption color={role.color}>
                                    {role.name}
                                </RoleOption>
                            ),
                        }))}
            ...
            >
                ...
                <H2 style={{ marginTop: "1rem" }}>
                    ...
                    {roles.length - roleOptions.length - 1}
                </H2>
                <ul>
