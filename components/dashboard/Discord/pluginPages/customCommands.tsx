...
const defaultCommand = (serverId: string): command => ({
    DM: false,
    allowedChannels: [],
    bannedRoles: [],
    cooldown: 0,
...
export const CommandModal = ({ defaultValue, ...props }) => {
    ...
    const [state, dispatch] = useReducer<
        (state: command, action: action) => command,
        command
    >(
        commandReducer,
        ...
        <Modal open={props.open} onClose={props.onClose}>
            <CommandModalBody>
                ...
                <CreateCommandArea>
                    ...
                    <SectionSubtitle>
                        Anyone with these roles can use this command. Adding
                    ...
                    <Select
                        ...
                        options={roles
                            .filter(
                                ({ id }) => !state.permittedRoles.includes(id)
                            )
                            .map((role) => ({
                                value: transformObjectToSelectValue(role),
                                label: (
                    ...
                    <SectionSubtitle>
                        The command can only be used in these channels.
                        Selecting none means it can be used anywhere
                    </SectionSubtitle>
                    <Select
                        ...
                        options={allChannels.map((role) => ({
                            value: transformObjectToSelectValue(role),
                            label: (
                                <ChannelItem {...role}>{role.name}</ChannelItem>
                            ),
                        }))}
                        onChange={(value) => {
                            const roleId = parseSelectValue(value);
...
const CustomCommands = () => {
    ...
    const [commandBeingEdited, setCommandBeingEdited] =
        ...
        <div>
            ...
            <CommandsHeader>
                <span>
                    <H2>Text Command</H2>
                    <h4>
                        A simple command that responds with a custom message in
                        DM or public
                    </h4>
                </span>
                <span>
            ...
            <SaveBar
                changed={changed}
                save={save}
                reset={() => setLocalCommands(snapshot)}
            />
        </div>
    );
};
