...
import { isPremiumServer } from "../../utils/functions";
...
export const DiscordContextProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const fetchPremiumStatus = async () => {
            const premium = await isPremiumServer({ id: serverId });
        ...
        const fetchFromFirebase = async () => {
            const serverRef = firebaseClient.db
...
export const useDiscordContext = () => {
    ...
    if (!context) {
        throw new Error(
            "useDiscordContext must be used within a discord context provider"
        );
    }
    return context;
};
